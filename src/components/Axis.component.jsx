...
export default class HorisontalBarChart2 extends React.Component {
	...
	createXAxis() {
		const xAxis = d3.axisTop(this.props.scaleX).tickSize(this.props.scaleY.range()[0]).tickFormat((d) => d);
		...
		d3.select(this.axisXRef.current).call((g) => {
			...
			g.selectAll('.tick line').attr('y1', 5).classed('chart-axis__axis', true);
	...
	createYAxis() {
		const yAxis = d3.axisRight(this.props.scaleY).tickSize(this.props.scaleX.range()[1]).tickFormat((d) => d);

		d3.select(this.axisYRef.current).call((g) => {
			g.call(yAxis);
			g.selectAll('.tick line').attr('x1', -5).classed('chart-axis__axis', true);

			g.selectAll('.tick text').attr('x', -10).classed('chart-axis__label', true);
